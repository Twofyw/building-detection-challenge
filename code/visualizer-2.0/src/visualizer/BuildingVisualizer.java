...
public class BuildingVisualizer implements ActionListener, ItemListener, MouseListener {
	...
	private void run() {
		idToSolutionPolygons = load(new String[] {solutionPath}, false);
		idToTruthPolygons = load(truthPaths, true);
		...
		if (idToSolutionPolygons.isEmpty() || idToTruthPolygons.isEmpty()) {
			...
			for (String c: cities) cityToScore.put(c, new Metrics());
    ...
    private Map<String, Polygon[]> load(String[] paths, boolean truth) {
